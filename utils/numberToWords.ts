export function numberToWords(num: number): string {
    ...
    const stringNum = String(num.toFixed(2));
    const parts = stringNum.split(".");
    
    // Convert first part to words
    const intPart = parseInt(parts[0], 10);
    const words = intPart === 0 ? 'CERO' : Millones(intPart);
    
    const floatPart = parts[1] || '00';
    
    return `SON: ${words.trim()} CON ${floatPart}/100 SOLES`;
}
