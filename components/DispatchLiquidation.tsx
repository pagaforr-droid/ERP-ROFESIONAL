...
export const DispatchLiquidationComp: React.FC = () => {
  ...
  const confirmPartial = () => {
     ...
     // Generate Mock NC Number
     const mockNC = `NC01-${Math.floor(Math.random() * 10000).toString().padStart(6, '0')}`;
        ...
        <div className="h-full flex flex-col bg-slate-100 relative">
           ...
           <div className="flex-1 overflow-auto p-4 space-y-3">
              {dispatchSales.map(sale => {
                 ...
                 return (
                    <div key={sale.id} className={`bg-white p-4 rounded-lg shadow-sm border-l-4 flex flex-col gap-3 ${
                       status.action === 'PAID' ? 'border-green-500' :
