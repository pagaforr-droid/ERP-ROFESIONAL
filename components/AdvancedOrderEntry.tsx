...
export const AdvancedOrderEntry: React.FC = () => {
   ...
   const [series, setSeries] = useState(company.series.find(s => s.type === 'PEDIDO' && s.is_active)?.series || 'PE01');
   const [docNumber, setDocNumber] = useState(String(company.series.find(s => s.type === 'PEDIDO' && s.is_active)?.current_number || 1).padStart(8, '0'));

   // Update series when doc type changes
   useEffect(() => {
      const ser = company.series.find(s => s.type === 'PEDIDO' && s.is_active);
      if (ser) {
   ...
   const filteredOrders = orders.filter(o => o.status === 'pending').filter(o =>
      (o.client_name || "").toLowerCase().includes(orderSearchTerm.toLowerCase()) ||
      (o.code || "").includes(orderSearchTerm) ||
      (o.client_doc_number || "").includes(orderSearchTerm)
   );
   ...
   const handleNewOrder = () => {
      ...
      const activePedidoSeries = company.series.find(s => s.type === 'PEDIDO' && s.is_active);
      setDocType('FACTURA'); // default suggested doc type
      if (activePedidoSeries) {
         setSeries(activePedidoSeries.series);
         setDocNumber(String(activePedidoSeries.current_number).padStart(8, '0'));
      } else {
         setSeries('');
         setDocNumber('');
      }
   ...
   const loadOrder = (order: Order, mode: 'VIEW' | 'EDIT' = 'VIEW') => {
      if (mode === 'EDIT' && (order.status === 'processed' || order.status === 'rejected')) {
         alert('No se puede modificar un comprobante enviado o aceptado por SUNAT.');
         return;
      }
      setIsViewMode(mode === 'VIEW');
      setIsEditMode(mode === 'EDIT');
      if (mode === 'EDIT') setOriginalOrder(order);

      setDocType(order.suggested_document_type as any);
   ...
   const handlePreview = () => {
      // Construct a temporary order object for preview
      const tempSale: Order = {
         id: 'preview',
         code: docNumber || 'NUEVO',
         ...
         items: cart as any // UI mapping
      };
      setOrderToPrint(tempSale as any);
   };
   ...
   const selectClient = (c: Client) => {
      ...
      // Logic: Auto-select suggested document type based on RUC length
      const newDocType: 'FACTURA' | 'BOLETA' = c.doc_number.length === 11 ? 'FACTURA' : 'BOLETA';
      setDocType(newDocType);

      // Note: We DO NOT update 'series' here because 'series' in this component refers
      // to the PEDIDO series, while newDocType is the SUGGESTED doc type for the final sale.
      // Order correlative generation will use the currently selected PEDIDO series.
   ...
   const handleSaveSale = () => {
      if (isViewMode && !isEditMode) {
         ...
         const currentOrder = orders.find(o => o.code === docNumber);
         if (currentOrder) setOrderToPrint(currentOrder);
         return;
      }

      if (cart.length === 0) return;
      if (!selectedClientId && !clientData.name) { alert("Ingrese datos del cliente"); return; }
      if (!series) { alert("No hay una serie asignada a este tipo de documento. Configure una en los Ajustes de Empresa para 'PEDIDO'."); return; }

      // --- CONFIRMATION MODAL ---
      const confirmSave = window.confirm(`¿Está seguro que desea ${isEditMode ? 'MODIFICAR' : 'emitir'} el pedido con doc sugerido ${docType} y serie ${series}?`);
      if (!confirmSave) return;

      let orderCode = 'PED-ERROR';
      if (isEditMode && originalOrder) {
         orderCode = originalOrder.code;
      } else {
         const next = getNextDocumentNumber('PEDIDO', series);
         if (!next) {
            alert("Error al generar el correlativo del pedido.");
            return;
         }
         orderCode = `${next.series}-${next.number}`;
      }
      ...
      const newOrderData: Order = {
         ...
         items: cart.map(item => ({
            ...
            quantity: item.quantity_presentation,
      ...
      handleNewOrder(); // automatically resets to next doc number
   };
      ...
      <div className="flex flex-col h-full bg-slate-200 p-2 font-sans text-xs relative">
         ...
         <div className="bg-white p-2 rounded shadow-sm border border-slate-300 mb-2 space-y-2 relative">
            ...
            <div className="absolute top-2 right-2 flex gap-2">
               <button onClick={handleNewOrder} className="bg-slate-700 text-white px-3 py-1 rounded shadow text-xs font-bold hover:bg-slate-600 flex items-center">
            ...
            <div className="flex items-center gap-3 mt-1 py-1">
               <div className="flex items-center gap-2 bg-slate-50 px-2 py-1.5 rounded border border-slate-200 shadow-sm">
                  <label className="font-bold text-slate-700 text-sm">Doc. Final Sugerido</label>
                  <select
               ...
               <div className="flex items-center gap-2 bg-slate-50 px-2 py-1.5 rounded border border-slate-200 shadow-sm">
                  <label className="font-bold text-slate-700 text-sm">Nro. Pedido (Serie)</label>
                  <select
                     ...
                     onChange={(e) => {
                        ...
                        const sObj = company.series.find(s => s.type === 'PEDIDO' && s.series === e.target.value);
                        if (sObj) setDocNumber(String(sObj.current_number).padStart(8, '0'));
                     }}
                     disabled={isViewMode}
                  >
                     {company.series.filter(s => s.type === 'PEDIDO' && s.is_active).map(s => (
                        <option key={s.id} value={s.series}>{s.series}</option>
                     ))}
                  </select>
                  <span className="w-24 text-center text-sm font-mono text-slate-800 bg-transparent px-1 font-bold pointer-events-none">
                    {isEditMode ? docNumber : (docNumber || 'AUTOGENERADO')}
                  </span>
               </div>
               <div className="flex items-center gap-2 bg-slate-50 px-2 py-1.5 rounded border border-slate-200 shadow-sm">
         ...
         <div className="h-24 bg-slate-100 border-t border-slate-400 flex p-2 gap-4">
            ...
            <button
               ...
               disabled={isViewMode && !orderToPrint}
               className={`w-32 bg-green-600 hover:bg-green-700 text-white font-bold rounded shadow-lg flex flex-col items-center justify-center ${(isViewMode && !orderToPrint) ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
               ...
               <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                  <div className="bg-white w-full max-w-5xl rounded-lg shadow-2xl flex flex-col max-h-[85vh]">
                     <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-100 rounded-t-lg">
                        <h3 className="font-bold text-slate-700 flex items-center text-lg"><Search className="w-5 h-5 mr-2" /> Buscar Pedido</h3>
                           ...
                           <tbody className="divide-y divide-slate-100">
                              {filteredOrders.map(o => (
                                 <tr key={o.id} className="hover:bg-blue-50 transition-colors">
                                    <td className="p-4">
                                       <span className="font-bold text-blue-700 text-[15px]">{o.code}</span>
                                    </td>
                                    <td className="p-4">
                                       <span className="text-slate-600 font-medium text-[15px]">{new Date(o.created_at).toLocaleDateString()}</span>
                                    </td>
                                    <td className="p-4">
                                    ...
                                    <td className="p-4 text-right">
                                       <span className="font-black text-slate-900 text-[16px]">S/ {o.total.toFixed(2)}</span>
                                    </td>
                                    <td className="p-4 text-center">
                                       <span className={`px-3 py-1 rounded text-xs font-bold ring-1 ${o.status === 'processed' ? 'bg-green-100 ring-green-300 text-green-800' : 'bg-slate-100 ring-slate-300 text-slate-800'}`}>{o.status || 'PENDING'}</span>
                                    </td>
                                    <td className="p-4 text-right flex gap-2 justify-end">
                                       <button
                                          onClick={() => setShowHistoryModal({ isOpen: true, order: o })}
                                       ...
                                       <button
                                          onClick={() => requestAdminAuth(() => loadOrder(o, 'EDIT'), 'Modificar Documento')}
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
               <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-2xl max-h-[80vh] flex flex-col">
                  <div className="flex justify-between items-center bg-slate-100 rounded-t-lg mb-4">
                     <h3 className="font-bold text-slate-800 text-lg flex items-center"><Eye className="w-5 h-5 mr-2 text-slate-500" /> Historial de Pedido: {showHistoryModal.order.code}</h3>
