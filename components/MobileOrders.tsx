...
export const MobileOrders: React.FC = () => {
   ...
   const handleAddProduct = () => {
      ...
      // Check for existing product in cart to prevent duplicates directly
      const existingIndex = cart.findIndex(item => item.product_id === selectedProd.id && !item.is_promo);
      if (existingIndex !== -1) {
         alert(`ATENCIÓN: Este producto ya se encuentra en el pedido. Para aplicar o acumular correctamente las bonificaciones, por favor edite la cantidad del ítem que ya está en su carrito.`);
         return;
      }
   ...
   const updateCartItemQty = (index: number, delta: number) => {
      ...
      // Prevent editing promo items
      if (newCart[index].is_promo) return;
      ...
      if (newQty <= 0) {
         ...
         // Optionally you could check for stock again here
                  ...
                  <div className="flex-1 overflow-auto p-4 space-y-3 pb-32">
                     {cart.length === 0 ? (
                        ...
                        cart.map((item, idx) => {
                           ...
                           if (item.is_promo) {
                              return (
                                 <div key={idx} className="bg-green-50 p-3 rounded-lg shadow-sm border border-green-200 flex justify-between items-center animate-fade-in-up">
                                    <div>
                                       <div className="font-medium text-sm text-green-800 flex items-center">
                           ...
                           return (
                              <div key={idx} className="bg-white border text-sm border-slate-200 rounded-lg overflow-hidden shadow-sm">
                                 <div className="flex justify-between items-start p-3 bg-slate-50 border-b border-slate-100">
                                    <div className="font-bold text-slate-800 flex-1 pr-2 leading-tight">{item.product_name}</div>
                                    <button onClick={() => {
                                 ...
                                 <div className="p-3 flex items-center justify-between">
                                    <div className="flex items-center gap-2 bg-slate-100 rounded-lg p-1 border border-slate-200">
                                       <button onClick={() => updateCartItemQty(idx, -1)} className="w-8 h-8 flex items-center justify-center bg-white text-slate-600 rounded shadow-sm hover:bg-slate-50 hover:text-blue-600 transition-colors">
                                          <Minus className="w-4 h-4" />
                                       </button>
                                       <div className="text-center font-bold text-slate-800 min-w-[3rem]">
                                          <div className="text-lg leading-none">{item.quantity}</div>
                                          <div className="text-[9px] uppercase text-slate-500 pt-0.5">{unitLabel}</div>
                                       </div>
                                       <button onClick={() => updateCartItemQty(idx, 1)} className="w-8 h-8 flex items-center justify-center bg-white text-slate-600 rounded shadow-sm hover:bg-slate-50 hover:text-blue-600 transition-colors">
                                    ...
                                    <div className="text-right">
                                       <div className="text-xs text-slate-400 font-medium pb-0.5">S/ {item.unit_price.toFixed(2)} c/u</div>
                                       <div className="font-bold text-slate-900 text-lg leading-none">S/ {item.total_price.toFixed(2)}</div>
                                    </div>
                                 </div>
                              </div>
                           );
