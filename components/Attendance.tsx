...
import { Clock, User as UserIcon, LogIn, LogOut, Calendar, BarChart2, Lock, UserCheck, CheckCircle2, AlertCircle, X, Edit, Save, Camera, MapPin } from 'lucide-react';
...
export const Attendance: React.FC = () => {
   ...
   const handleAuthSubmit = async (e: React.FormEvent) => {
      ...
      try {
         // 2. Capture Photo
         const photoBase64 = webcamRef.current?.getScreenshot() || undefined;

         // 3. Capture Location (Wrap in promise)
         ...
         if (navigator.geolocation) {
            try {
               ...
               console.warn("Could not get precise location, using Cusco fallback", locErr);
               // Use Cusco, Peru as fallback coordinates as requested by user
         ...
         if (authModal.mode === 'IN') {
            clockIn(authModal.userId, photoBase64, location);
         } else {
            clockOut(authModal.userId, photoBase64, location);
         }
         ...
         console.error(error);
         setErrorMsg("Error al registrar asistencia. Revise permisos de cámara/ubicación.");
      } finally {
                     ...
                     {attendableUsers.map(user => {
                        ...
                        return (
                           <button
                              ...
                              className={`relative group flex flex-col items-center p-6 rounded-xl border-2 shadow-sm transition-all transform hover:scale-105 hover:shadow-xl ${isOnShift
                           ...
                           >
                              ...
                              {isOnShift ? (
                                 <div className="bg-green-50 text-green-800 px-3 py-1 rounded-full flex flex-col items-center border border-green-200">
                                    ...
                                    <div className="text-[10px] text-green-600 font-mono">
                                       En curso: {formatDuration(currentTime.getTime() - new Date(activeRecord.check_in).getTime())}
                                    </div>
                                 </div>
                              ) : (
                        ...
                        <form onSubmit={handleAuthSubmit} className="p-8">
                           ...
                           <div className="grid grid-cols-2 gap-3">
                              ...
                              <button
                                 ...
                                 className={`py-3 rounded-lg font-bold text-white shadow-lg transition-transform active:scale-95 ${authModal.mode === 'IN' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'} ${isLoadingAuth ? 'opacity-70 cursor-wait' : ''}`}
                              >
                                 {isLoadingAuth ? 'PROCESANDO...' : 'CONFIRMAR'}
                              </button>
                           </div>
                        </form>
                     </div>
                  ...
                  <table className="w-full text-left text-sm">
                     <thead className="bg-slate-100 text-slate-600 font-bold sticky top-0">
                        <tr>
                           ...
                           <th className="p-3 text-center">Evidencia Entrada</th>
                           <th className="p-3 text-center">Hora Salida</th>
                           <th className="p-3 text-center">Evidencia Salida</th>
                              ...
                              <tr key={r.id} className="hover:bg-slate-50 group">
                                 ...
                                 <td className="p-3">
                                    <div className="flex flex-col items-center gap-1">
                                       {r.photo_in ? <img src={r.photo_in} className="w-10 h-10 object-cover rounded border border-slate-200 shadow-sm" alt="IN" /> : <span className="text-[10px] text-slate-400">Sin Foto</span>}
                                       {r.location_in ? <a href={`https://maps.google.com/?q=${r.location_in.lat},${r.location_in.lng}`} target="_blank" rel="noreferrer" className="text-blue-500 hover:text-blue-700"><MapPin className="w-4 h-4" /></a> : null}
                                    </div>
                                 </td>
                                 <td className="p-3 text-center text-red-700 font-medium bg-red-50/50 rounded">{formatTime(r.check_out)}</td>
                                 <td className="p-3">
                                    <div className="flex flex-col items-center gap-1">
                                       {r.photo_out ? <img src={r.photo_out} className="w-10 h-10 object-cover rounded border border-slate-200 shadow-sm" alt="OUT" /> : <span className="text-[10px] text-slate-400">{r.status === 'CLOSED' ? 'Sin Foto' : '-'}</span>}
                                       {r.location_out ? <a href={`https://maps.google.com/?q=${r.location_out.lat},${r.location_out.lng}`} target="_blank" rel="noreferrer" className="text-blue-500 hover:text-blue-700"><MapPin className="w-4 h-4" /></a> : null}
                                    </div>
                                 </td>
