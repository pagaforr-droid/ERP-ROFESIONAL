...
export const GuiaManager: React.FC = () => {
    ...
    const pendientes = useMemo(() => {
        ...
        return filtered;
    }, [realSales, searchTerm, dateFilter]);
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {activeTab === 'pendientes' && pendientes.map((sale) => (
                                <tr key={sale.id} className={`hover:bg-emerald-50/50 transition-colors ${selectedIds.has(sale.id) ? 'bg-emerald-50' : 'bg-white'}`}>
                                    ...
                                    <td className="p-4 text-sm text-slate-600">{new Date(sale.created_at).toLocaleDateString()}</td>
                                    <td className="p-4">
                                        <div className="font-bold text-slate-800">{sale.document_type}</div>
                                        <div className="text-xs text-slate-500">{sale.series}-{sale.number}</div>
                                    </td>
                                    <td className="p-4">
                                    ...
                                    <td className="p-4 text-right font-bold text-slate-800">S/ {sale.total.toFixed(2)}</td>
                                </tr>
                            ))}
